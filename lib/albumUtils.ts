...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillText('Generated with Rockstar Portrait', canvasWidth / 2, 150);
    ...
    const guitarModels = Object.keys(imageData);
    const loadedImages = await Promise.all(
        Object.values(imageData).map(url => loadImage(url))
    );

    const imagesWithModels = guitarModels.map((model, index) => ({
        model,
    ...
    const reversedImages = [...imagesWithModels].reverse();
    reversedImages.forEach(({ model, img }, reversedIndex) => {
        // Calculate the original index to determine grid position
        const index = imagesWithModels.length - 1 - reversedIndex;
